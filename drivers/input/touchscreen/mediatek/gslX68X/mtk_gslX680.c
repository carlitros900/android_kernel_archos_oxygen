...
{
#ifdef I2C_SMBUS_FORCE
	int ret;
	ret = i2c_smbus_read_i2c_block_data(client, addr, len, rxbuf); 	
	return ret;
#else
	int left = len;
	int readLen = 0;
...
static s32 gsl_i2c_write_bytes(struct i2c_client *client, u8 addr, u8 *txbuf, int len)
{
	...
	if (ret != 0 ) {
		GSL_LOGD("gsl_carga_fw (ERROR): %d 0x%04X, %d byte(s) \n", ret, addr, len);
		GSL_LOGD("gsl_carga_fw (ERROR): 0x%04X \n", txbuf[0]);
	}
	return 0;
#else
	int ret = 0;
...
#define DRV_NUM                 28
#define SEN_NUM                 18
#define RAWDATA_THRESHOLD       9000
#define DAC_THRESHOLD           20
static const u8 sen_order[SEN_NUM] = {21, 20, 17, 16, 15, 14, 13, 12, 11, 10, 9, 8, 7, 6, 4, 1, 0, 0};
