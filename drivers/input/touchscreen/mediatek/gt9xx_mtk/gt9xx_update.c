...
printk("gup_enter_update_mode\n");
		...
		//
		gtp_gpio_input(GTP_IRQ_GPIO);//chen
		while (retry++ < 200) {
			/* step4:Hold ss51 & dsp */
		...
		printk("gup_enter_update_mode  pass\n");
...
static u8 gup_enter_update_judge(struct st_fw_head *fw_head)
{
		...
		GTP_INFO("chen:FILE HARDWARE INFO:%02x%02x%02x%02x",
		fw_head->hw_info[4], fw_head->hw_info[5], fw_head->hw_info[6], fw_head->hw_info[7]);
		GTP_INFO("chen:FILE HARDWARE INFO:%02x%02x%02x%02x",
		fw_head->hw_info[8], fw_head->hw_info[9], fw_head->hw_info[10], fw_head->hw_info[11]);
...
static u8 gup_check_update_file(struct i2c_client *client, struct st_fw_head *fw_head, u8 *path)
{
		...
		if (path) {
			...
			printk("Firmware actual size: %d(%dK)",
...
s32 gup_update_proc(void *dir)
{
		...
		while (retry++ < 5) {
			...
			printk("[update_proc]UPDATE SUCCESS.\n");			
			break;
		}
