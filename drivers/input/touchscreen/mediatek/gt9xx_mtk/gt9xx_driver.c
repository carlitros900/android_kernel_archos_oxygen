...
void gtp_gpio_output(int gpio_type, int level)
{
	...
	if (gpio_type == GTP_RST_GPIO)
		...
		mt_gpio_opt_mode(51,0);
		mt_gpio_opt_dir(51,0);
		mt_gpio_opt_output(51,level);
		}
#endif
}
...
static int tpd_irq_registration(void)
{
	...
	int ret = 0;
	gtp_gpio_input(GTP_IRQ_GPIO);
	printk("chensenbin:into tpd_irq_registration\n");
	node = of_find_compatible_node(NULL, NULL, "mediatek,goodix_touch");
//	node = of_find_matching_node(NULL, touch_of_match);//0704
	if (node) {
