...
int force_get_tbat(kal_bool update)
{
	...
	return 25;
#elif defined(SOC_BY_EXT_HW_FG) && defined(EXT_FG_HAS_TEMPERATURE)
         
#if defined(CONFIG_MTK_BQ27520_SUPPORT)
...
#if defined(SOC_BY_EXT_HW_FG) && defined(EXT_FG_HAS_TEMPERATURE)
#if defined(CONFIG_MALATA_HARDWARE_VERSION)
	if (MATCH_BQ27520_HARDWARE_VERSION == hardware_version){
#ifdef CONFIG_MTK_BQ27520_SUPPORT
		if(!tempeture_num)
		{
			bq27520_write_batt_temperature(bat_temperature_val);
			g_battery_tempeture = bat_temperature_val;
			tempeture_num ++;
		} else{
			if(((bat_temperature_val - g_battery_tempeture) >10)  ||((g_battery_tempeture - bat_temperature_val) > 10))
			{
				bq27520_write_batt_temperature(bat_temperature_val);
				g_battery_tempeture = bat_temperature_val;
			}
...
signed int battery_meter_get_battery_voltage(kal_bool update)
{
...
#if defined(SOC_BY_EXT_HW_FG) && defined(EXT_FG_HAS_VOLTAGE)
	kal_int32 arg = EXT_FG_CMD_VOLTAGE;
	...
	if (battery_meter_ctrl(BATTERY_METER_CMD_EXT_HW_FG, &arg) == 0){
		...
		printk("[battery_meter_get_battery_voltage] read bq27520 voltage error!!!\n");
	}
#if defined(CONFIG_MALATA_HARDWARE_VERSION)
	}
#endif
...
signed int battery_meter_get_battery_current(void)
{
...
#if defined(SOC_BY_EXT_HW_FG) && defined(EXT_FG_HAS_CURRENT)
	kal_int32 arg = EXT_FG_CMD_CURRENT;
	...
	if (battery_meter_ctrl(BATTERY_METER_CMD_EXT_HW_FG, &arg) == 0)
		...
		printk("[battery_meter_get_current] read bq27520 current error!!!\n");
#if defined(CONFIG_MALATA_HARDWARE_VERSION)
	}
#endif
#endif
...
#if defined(SOC_BY_EXT_HW_FG) && defined(EXT_FG_HAS_SOC)
...
#if defined(BAT_CAPACITY_MIN) && defined(BAT_CAPACITY_MAX)
		arg = max(arg, BAT_CAPACITY_MIN);
		arg = ((arg - BAT_CAPACITY_MIN) * 100 + (BAT_CAPACITY_MAX - BAT_CAPACITY_MIN) / 2) / (BAT_CAPACITY_MAX - BAT_CAPACITY_MIN);
#endif

		return arg;
	}else{
		printk("[battery_meter_get_battery_percentage] read bq27520 battery capacity error!!!\n");
	}
#if defined(CONFIG_MALATA_HARDWARE_VERSION)
	}
#endif
...
{
#if defined(SOC_BY_EXT_HW_FG) && defined(EXT_FG_HAS_SHUTDOWN)
    kal_int32 arg = 0;
#endif
	bm_print(BM_LOG_CRTI, "[battery_meter_shutdown]\n");

#if defined(SOC_BY_EXT_HW_FG) && defined(EXT_FG_HAS_SHUTDOWN)
	arg = EXT_FG_CMD_SHUTDOWN;
	...
	if (battery_meter_ctrl(BATTERY_METER_CMD_EXT_HW_FG, &arg) == 0)
		...
		printk("[battery_meter_shutdown] write bq27520 shutdown error!!!\n");
#if defined(CONFIG_MALATA_HARDWARE_VERSION)
	}
#endif
#endif
