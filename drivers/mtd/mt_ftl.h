...
#define NAND_RESERVED_PEB               3 /* for WL/Scrub max:3, miin:1*/
...
#define NAND_RESERVED_FTL               (NAND_RESERVED_PEB + DATA_START_BLOCK + 3) /* reserved for ftl*/
...
#define mt_ftl_assert(expr, buf, size)  do {                                               \
	if (unlikely(!(expr))) {                                             \
		int i;	\
		pr_crit("FTL assert failed in %s at %u (pid %d)\n",          \
		       __func__, __LINE__, current->pid);                    \
		dump_stack(); \
		for (i = 0; i < size; i++) { \
			pr_crit("%02x ", *((char *)buf + i)); \
			if ((i+1) % 16 == 0) \
				pr_crit("\n"); \
		} \
	}                                                                    \
...
int mt_ftl_updatePMT(struct mt_ftl_blk *dev, unsigned int cluster, int sec_offset, int leb, int offset, int part,
