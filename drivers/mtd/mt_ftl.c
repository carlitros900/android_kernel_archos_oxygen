...
int mt_ftl_compress(struct mt_ftl_blk *dev, const void *in_buf, int in_len, int *out_len)
{
	int ret = 0;
...
int mt_ftl_decompress(struct mt_ftl_blk *dev, const void *in_buf, int in_len, int *out_len)
{
	int ret = 0;
	struct mt_ftl_param *param = dev->param;
	...
	if (in_len == FS_PAGE_SIZE) {
		...
		*out_len = in_len;
		...
		ret =
		    crypto_comp_decompress(param->cc, in_buf, in_len, param->cmpr_page_buffer, out_len);
		if (ret) {
			mt_ftl_err(dev, "ret = %d, out_len = %d, in_len = 0x%x",
					ret, *out_len, in_len);
			mt_ftl_err(dev, "cc = 0x%lx, buffer = 0x%lx",
					(unsigned long int)param->cc, (unsigned long int)in_buf);
			mt_ftl_err(dev, "cmpr_page_buffer = 0x%lx",
					(unsigned long int)param->cmpr_page_buffer);
			return MT_FTL_FAIL;
		}
	}
	return ret;
...
void mt_ftl_leb_map(struct ubi_volume_desc *desc, int lnum)
{
	int ret;

	ret = ubi_is_mapped(desc, lnum);
	if (ret == 0) {
		ubi_err("leb %d is unmapped", lnum);
		...
		unsigned int u4DstLeb, int *offset_dst, bool isReplay)
{
	...
	char *gc_pmt_buffer = vmalloc(NAND_PAGE_SIZE * 2);

	if (!isReplay)
		mt_ftl_leb_map(desc, u4DstLeb);
	...
	for (i = 0; i < (PMT_TOTAL_CLUSTER_NUM); i++) {
		if (PMT_INDICATOR_GET_BLOCK(param->u4PMTIndicator[i]) == u4SrcLeb) {
			...
			if (!isReplay) {
				/* Copy PMT & META PMT */
				ret = ubi_leb_read(desc, u4SrcLeb, gc_pmt_buffer, offset_src, NAND_PAGE_SIZE * 2, 0);
				...
				ret = ubi_leb_write(desc, u4DstLeb, gc_pmt_buffer, *offset_dst, NAND_PAGE_SIZE * 2);
				if (ret) {
					mt_ftl_err(dev,
		   ...
		   int *offset_dst, bool isReplay)
{
	...
	struct ubi_volume_desc *desc = dev->desc;
	struct mt_ftl_param *param = dev->param;
/* fix gc data all valid pages of block*/
	struct mt_ftl_data_header valid_header;
	int valid_data_offset = 0;
	int valid_header_offset = 0;
	int valid_data_num = 0;
	unsigned int total_consumed_size = 0;
	unsigned int offset_in_page = 0, data_len = 0;
	char *valid_buffer = vmalloc(NAND_PAGE_SIZE);
/* end of fix */

	const int max_offset_per_block = dev->leb_size - NAND_PAGE_SIZE;
	...
	*offset_dst = 0;
	...
	andSec = NAND_DEFAULT_SECTOR_VALUE;
	for (i = 0; i < data_num; i++)
		andSec &= header_buffer[i].sector;

	if (andSec == NAND_DEFAULT_SECTOR_VALUE) {
	...
	while ((andSec != NAND_DEFAULT_SECTOR_VALUE) && (offset_src <= max_offset_per_block)) {
#ifdef PROFILE
		start_time = getnstimenow();
#endif				/* PROFILE */
		...
		for (i = 0; i < data_num; i++) {
			...
			mt_ftl_assert(cluster <= PMT_TOTAL_CLUSTER_NUM, param->gc_page_buffer + 3*1024, 4*1024);
			if (cluster >= PMT_TOTAL_CLUSTER_NUM)
				mt_ftl_err(dev, "sector(0x%lx) cluster(0x%x) i(%d) data_num(%d)", sector, cluster,
					i, data_num);
			...
			if (!isReplay) {
				if ((u4SrcLeb == PMT_GET_BLOCK(pmt)) &&
					...
					total_consumed_size = valid_data_offset + data_len +
						(valid_data_num + 1) * sizeof(struct mt_ftl_data_header) + 4;
					if ((total_consumed_size > NAND_PAGE_SIZE) ||
							valid_data_num >= MTKFTL_MAX_DATA_NUM_PER_PAGE) {
						/* invalid_num = NAND_PAGE_SIZE - valid_data_offset
						   - valid_data_num * sizeof(struct mt_ftl_data_header) - 4; */
						ret = ubi_leb_write(desc, u4DstLeb, valid_buffer, *offset_dst,
								NAND_PAGE_SIZE);
						if (ret) {
							mt_ftl_err(dev,
						...
						valid_data_offset = 0;
						valid_data_num = 0;
						memset(valid_buffer, 0xFF, NAND_PAGE_SIZE * sizeof(char));
						*offset_dst += NAND_PAGE_SIZE;
						page_dst++;
					}
					mt_ftl_updatePMT(dev, cluster, sec_offset, u4DstLeb,
							 page_dst * NAND_PAGE_SIZE, valid_data_num,
							 data_len, isReplay, 0);
					/* mt_ftl_err(dev,
					   "sector(0x%lx), DstLeb(%d), page_dst(%d), valid_data_num(%d),
					   data_len(%d)\n",sector, u4DstLeb, page_dst, valid_data_num, data_len); */
					valid_header.offset_len = (valid_data_offset << 16) | data_len;
					valid_header.sector = sector;
					memcpy(&valid_buffer[valid_data_offset],
							(char *)param->gc_page_buffer + offset_in_page, data_len);
					valid_data_offset += data_len;
					valid_header_offset = NAND_PAGE_SIZE -
						(valid_data_num + 1) * sizeof(struct mt_ftl_data_header) - 4;
					memcpy(&valid_buffer[valid_header_offset],
							&valid_header, sizeof(struct mt_ftl_data_header));
					valid_data_num++;
					memcpy(&valid_buffer[NAND_PAGE_SIZE - 4], &valid_data_num, 4);
				}
			} else
				mt_ftl_updatePMT(dev, cluster, sec_offset, u4DstLeb,
	...
	if (valid_data_num) {
		ret = ubi_leb_write(desc, u4DstLeb, valid_buffer, *offset_dst, NAND_PAGE_SIZE);
		if (ret) {
			mt_ftl_err(dev,
					"ubi_leb_write failed, leb = %d, offset = %d ret = 0x%x\n",
					u4DstLeb, *offset_dst, ret);
		...
		/* invalid_num = NAND_PAGE_SIZE - total_consumed_size;
		   MT_FTL_PARAM_LOCK(dev);
		   BIT_UPDATE(param->u4BIT[u4DstLeb], invalid_num);
		   MT_FTL_PARAM_UNLOCK(dev);*/
		*offset_dst += NAND_PAGE_SIZE;
				 ...
				 unsigned int u4EndLeb, bool *allInvalid)
{
	...
	for (i = u4StartLeb; i < u4EndLeb; i++) {
		if (param->u4BIT[i] == (dev->leb_size - 4)) {
			u4ReturnLeb = i;
			*allInvalid = true;
			MT_FTL_PARAM_UNLOCK(dev);
		...
		if (param->u4BIT[i] > u4MaxInvalidVolume) {
			...
			u4ReturnLeb = i;
			mt_ftl_err(dev, "BIT[%d] = %d(%d)(%d:%d)", u4ReturnLeb, param->u4BIT[u4ReturnLeb],
				u4MaxInvalidVolume, u4StartLeb, u4EndLeb);
		}
	}
...
int mt_ftl_gc(struct mt_ftl_blk *dev, int *updated_page, bool isPMT, bool isReplay, int *isCommit)
{
	...
	if (unlikely(isPMT)) {
		...
		u4SrcInvalidLeb =
		    mt_ftl_gc_findblock(dev, PMT_START_BLOCK, DATA_START_BLOCK, &allInvalid);
		if (allInvalid)
			goto gc_end;
	} else {
...
gc_end:
	...
	*updated_page = offset_dst / NAND_PAGE_SIZE;
	if (*updated_page == page_num_per_block) {
		...
		for (i = PMT_START_BLOCK; i < desc->vol->ubi->volumes[dev->vol_id]->reserved_pebs - NAND_RESERVED_PEB;
		     i += 8) {
	...
	if (param->u4NextFreeLebIndicator != MT_INVALID_BLOCKPAGE) {
		...
		*updated_page = 0;
		...
		if (param->u4NextFreeLebIndicator >=
		    (desc->vol->ubi->volumes[0]->reserved_pebs - NAND_RESERVED_PEB - 1)) {
...
int mt_ftl_commitPMT(struct mt_ftl_blk *dev, bool isReplay, bool isCommitDataCache)
{
	...
	for (i = 0; i < PMT_CACHE_NUM; i++) {
		...
		pmt_page += 2;
		/*[Bean] pmt_page >= page_num_per_block not full write pmt???*/
		...
		if (param->i4CurrentReadPMTClusterInCache == param->i4CurrentPMTClusterInCache[i]) {
			mt_ftl_err(dev, "[Bean]clear read PMTCluster cache");
			param->i4CurrentReadPMTClusterInCache = 0xFFFFFFFF;
		}
		PMT_INDICATOR_RESET_INCACHE(param->u4PMTIndicator[param->i4CurrentPMTClusterInCache[i]]);
		param->i4CurrentPMTClusterInCache[i] = 0xFFFFFFFF;
...
int mt_ftl_downloadPMT(struct mt_ftl_blk *dev, unsigned int cluster, int cache_num)
{
	int ret = MT_FTL_SUCCESS, i = 0;
	int pmt_block = 0, pmt_page = 0;
...
int mt_ftl_updatePMT(struct mt_ftl_blk *dev, unsigned int cluster, int sec_offset, int leb, int offset,
...
/*	mt_ftl_updatePMT(dev, cluster, sec_offset, MT_INVALID_BLOCKPAGE, 0, 0, 0);
	...
	if (PMT_IS_DATA_INCACHE(meta_pmt)) {
		...
		offset_in_pagebuf = page * NAND_PAGE_SIZE + ((header_buffer->offset_len >> 16) & 0xFFFF);
...
int mt_ftl_replay_single_block(struct mt_ftl_blk *dev, int leb, int page)
{
	...
	if (NAND_PAGE_SIZE <= data_hdr_offset) {
		mt_ftl_err(dev, "NAND_PAGE_SIZE(%d) <= data_hdr_offset(%d) data_num(%d)",
			   NAND_PAGE_SIZE, data_hdr_offset, data_num);
	}
	header_buffer =
...
int mt_ftl_discard(struct mt_ftl_blk *dev, unsigned long sector, unsigned nr_sects)
{
	...
	for (i = 0; i < desc->vol->ubi->volumes[dev->vol_id]->reserved_pebs - NAND_RESERVED_PEB; i++)
		ubi_leb_unmap(desc, i);
	...
	param->u4GCReserveLeb = desc->vol->ubi->volumes[dev->vol_id]->reserved_pebs - NAND_RESERVED_PEB - 1;
