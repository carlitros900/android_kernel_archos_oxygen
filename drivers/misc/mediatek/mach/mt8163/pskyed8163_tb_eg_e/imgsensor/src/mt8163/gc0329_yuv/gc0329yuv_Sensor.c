...
#define GC0329_TEST_PATTERN_CHECKSUM (0xbd6238cd) //(0x98ee0dca)//(0x5593c632)// cyrille add  for yidao
#else
#define GC0329_TEST_PATTERN_CHECKSUM (0x6511d977) //(0x98ee0dca)//(0x5593c632)// cyrille add  for yidao
#endif
...
void GC0329_Sensor_Init(void)
{
	...
	GC0329_write_cmos_sensor(0x2b , 0x30);
	GC0329_write_cmos_sensor(0x2c , 0x2c);
	GC0329_write_cmos_sensor(0x2d , 0x2c); //BLK
	...
	GC0329_write_cmos_sensor(0xd2 , 0x26);//28 cyrille
	...
	GC0329_write_cmos_sensor(0x2d , 0x02);  //exp level 1 , 0x12.50fps
	...
	GC0329_write_cmos_sensor(0x31 , 0x04);  //exp level 3 , 0x7.14fps
...
UINT32 GC0329YUVSetTestPatternMode(kal_bool bEnable)
{
    SENSORDB("[GC0329YUVSetTestPatternMode] Test pattern enable:%d\n", bEnable);
    if(bEnable)
    {
        GC0329_write_cmos_sensor(0xfe,0x00); //bit[4]: 1 enable test pattern, 0 disable test pattern
		GC0329_write_cmos_sensor(0xfa,0x32);
		GC0329_write_cmos_sensor(0x40,0x00);
		GC0329_write_cmos_sensor(0x41,0x00);
		GC0329_write_cmos_sensor(0x42,0x00);
		GC0329_write_cmos_sensor(0x4f,0x00);
		GC0329_write_cmos_sensor(0x18,0x06);
		GC0329_write_cmos_sensor(0xfe,0x00);
		GC0329_write_cmos_sensor(0x77,0x40);
		GC0329_write_cmos_sensor(0x78,0x40);
		GC0329_write_cmos_sensor(0x79,0x40);
	...
	{
	    GC0329_write_cmos_sensor(0x4c,0x00);//bit[0]: 1 enable test pattern, 0 disable test pattern
	}
    return ERROR_NONE;
}
...
// cyrille add for yidao
		case SENSOR_FEATURE_SET_TEST_PATTERN:
			GC0329YUVSetTestPatternMode((BOOL) *pFeatureData16);
			break;
