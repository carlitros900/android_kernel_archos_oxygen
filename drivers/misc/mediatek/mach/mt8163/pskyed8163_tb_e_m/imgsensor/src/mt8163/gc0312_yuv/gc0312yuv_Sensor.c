...
#define GC0312_TEST_PATTERN_CHECKSUM (0x35294d9f)//(0x5593c632)// cyrille add  for yidao
...
void GC0312_Sensor_Init(void)
{
	...
	//GC0312_write_cmos_sensor(0x17, 0x17);//14 cyrille
	GC0312_write_cmos_sensor(0x17, 0x14);//14 cyrille
	...
	///////////////////   DNDD  /////////////////////
	///////////////////////////////////////////////// 
	GC0312_write_cmos_sensor(0x82, 0x0b); //1f
	GC0312_write_cmos_sensor(0x83, 0x08);//0b
	GC0312_write_cmos_sensor(0x89, 0xf8);//f0
	
	///////////////////////////////////////////////// 
	//////////////////   EEINTP  ////////////////////
	///////////////////////////////////////////////// 
	GC0312_write_cmos_sensor(0x8f, 0xff); 
	GC0312_write_cmos_sensor(0x90, 0x9f); 
	GC0312_write_cmos_sensor(0x91, 0x90);
	GC0312_write_cmos_sensor(0x92, 0x03); 
	GC0312_write_cmos_sensor(0x93, 0x03); 
	GC0312_write_cmos_sensor(0x94, 0x06); //05
	GC0312_write_cmos_sensor(0x95, 0x63);//65 
	GC0312_write_cmos_sensor(0x96, 0xf8); //f0
	...
	GC0312_write_cmos_sensor(0xd1, 0x2c); // 34 cyrille
	GC0312_write_cmos_sensor(0xd2, 0x2a); // 34 cyrille
	GC0312_write_cmos_sensor(0xd3, 0x3c); 
	...
	GC0312_write_cmos_sensor(0x0c, 0x20);//00 seaser
	GC0312_write_cmos_sensor(0x12, 0x52); 
	GC0312_write_cmos_sensor(0x13, 0x48);//38 
	...
	GC0312_write_cmos_sensor(0x0b, 0x11);//31
	...
	GC0312_write_cmos_sensor(0x08, 0x94);//2a cyrille
...
UINT32 GC0312GetSensorID(UINT32 *sensorID)
{
    ...
    if (*sensorID != GC0312_SENSOR_ID) {
        *sensorID = 0xFFFFFFFF; 
        ...
        printk("Read Sensor ID Success = 0x%04x\n", *sensorID); 
    return ERROR_NONE;    
}
...
UINT32 GC0312Open(void)
{
	...
	printk("GC0312_ Sensor Read ID OK \r\n");
	printk("GC0312 Read Sensor ID Success[open] = 0x%x\n", sensor_id); 
	GC0312_Sensor_Init();
	GC0312_Write_More_Registers();
...
BOOL GC0312_set_param_banding(UINT16 para)
{
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
			...
			GC0312_write_cmos_sensor(0x06, 0xb6); 
			GC0312_write_cmos_sensor(0x07, 0x00);
			GC0312_write_cmos_sensor(0x08, 0x94);//2a
...
BOOL GC0312_set_param_exposure(UINT16 para)
{
	...
	switch (para)
	{
		...
		case AE_EV_COMP_00:		
			GC0312_write_cmos_sensor(0xfe, 0x01);
			GC0312_write_cmos_sensor(0x13, 0x48);//38 cyrille
			GC0312_write_cmos_sensor(0xfe, 0x00);
		break;
		...
		case AE_EV_COMP_20:
			GC0312_write_cmos_sensor(0xfe, 0x01);
			GC0312_write_cmos_sensor(0x13, 0x58);
			GC0312_write_cmos_sensor(0xfe, 0x00);
		break;
		case AE_EV_COMP_30:
			GC0312_write_cmos_sensor(0xfe, 0x01);
...
// cyrille add for yidao
UINT32 GC0312YUVSetTestPatternMode(kal_bool bEnable)
{
    SENSORDB("[GC0312YUVSetTestPatternMode] Test pattern enable:%d\n", bEnable);
    if(bEnable)
    {
        GC0312_write_cmos_sensor(0xfe,0x00); //bit[4]: 1 enable test pattern, 0 disable test pattern
		...
		//GC0312_write_cmos_sensor(0x4d,0x88);

	}
	else
	{
	    //GC0312_write_cmos_sensor(0x4c,0x20);//bit[0]: 1 enable test pattern, 0 disable test pattern
	}
    return ERROR_NONE;
}

//cyrille add for yidao end
    ...
    case SENSOR_FEATURE_SET_VIDEO_MODE:    //  lanking
	 ...
	 // cyrille add for yidao
		case SENSOR_FEATURE_SET_TEST_PATTERN:
			GC0312YUVSetTestPatternMode((BOOL) *pFeatureData16);
			break;
