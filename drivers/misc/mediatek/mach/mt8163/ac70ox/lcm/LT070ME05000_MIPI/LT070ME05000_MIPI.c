...
static unsigned int GPIO_BACKLIGHT_EN;//add by tubao
...
static int lcm_get_gpio(struct device *dev)
{
	...
	pr_info("%s : GPIO_LCD_PWR_EN\n", __func__);
	//add by tubao add the backlight enable
	
	GPIO_BACKLIGHT_EN = of_get_named_gpio(node, "backlight_en_gpio", 0);
	pr_info("%s : \n", __func__);
...
static int lcm_get_vgp_supply(struct device *dev)
{
	...
	lcm_vgp_ldo = devm_regulator_get(dev, "reg-lcm-vgp");
		if (IS_ERR(lcm_vgp_ldo)) {
			...
			dev_err(dev, "failed to get reg-lcm-vgp, %d\n", ret);
			return ret;
		}
...
#if 0
static struct LCM_setting_table lcm_backlight_0_level_setting[] = {
	{0x51, 1, {0x00}},
	{REGFLAG_END_OF_TABLE, 0x00, {}}
};
#endif
...
#endif
    params->physical_width = 94.5;
    params->physical_height = 151.2;
}
...
printk("tubao ==>>> come here  %s  \n",__FUNCTION__);
	...
	MDELAY(20);
//	lcm_set_gpio_output(GPIO_BACKLIGHT_EN, 1);	
	pr_info("%s : lcm_set_gpio_output\n", __func__);
...
//    MDELAY(300);
//	push_table(lcm_backlight_0_level_setting, sizeof(lcm_backlight_level_setting) / sizeof(struct LCM_setting_table), 1);
}
...
static void lcm_suspend(void)
{
	...
	lcm_set_gpio_output(GPIO_BACKLIGHT_EN, 0);	
...
void lcm_setbacklight(unsigned int level)
{
	...
	unsigned int data_array[1]={0xff511500};
	data_array[0]=(data_array[0]&0x00ffffff)|(level<<24);
	printk("%s, kernel", __func__);
	//data_array[0]= 0x00023902;
	//data_array[1]= (level<<8)|0x51;	//0x00007f51;
	MDELAY(10);
	data_array[0]= 0x10511500;
	printk("level=%x,data_array[0]= %x\n",level,data_array[0]);
	...
	dsi_set_cmdq(data_array, 1, 1);
	read_reg_v2(0x52, buffer, 4);
	printk("backlight buffer=%x\n",buffer);
}
#else
static void lcm_setbacklight(unsigned int level)
{
	...
	printk("%s, xingkun>>>>kernel,%x\n", __func__,lcm_backlight_level_setting[0].para_list[0]);
