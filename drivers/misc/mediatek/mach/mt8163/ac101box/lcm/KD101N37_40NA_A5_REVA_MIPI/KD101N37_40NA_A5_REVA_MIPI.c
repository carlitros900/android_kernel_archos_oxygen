...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->dsi.horizontal_backporch = 132;//48;
	params->dsi.horizontal_frontporch = 24;//16;
	...
	params->dsi.cont_clock = 0; //非 continous mode
	params->dsi.clk_lp_per_line_enable=0; //每帧进入 LP mode
...
#if  0 //from vendor,side effest:配合esd check, resume 闪屏
data_array[0] = 0xC8001500;
dsi_set_cmdq(data_array, 1, 1);
data_array[0] = 0x42B01500;
dsi_set_cmdq(data_array, 1, 1);
...
//    int i =0;
    ...
    {
        printk( "nt35596_AUO lcm_esd_check buffer[0] = %d\n",buffer[0]);
        return 1 /*KAL_TRUE*/;
    } else {
        if(buffer[3] != 0x02) {//error data type is 0x02
        	printk( "nt35596_AUO lcm_esd_check buffer[3] = %d\n",buffer[3]);
             return 0 /*KAL_FALSE*/;
        } else {
             ...
             if(err_count >=2 ) {
                 err_count = 0;
                 printk( "nt35596_AUO lcm_esd_check buffer[4] = %d , buffer[5] = %d\n",buffer[4],buffer[5]);
                 return 1/*KAL_TRUE*/;
             }
                 printk( "nt35596_AUO lcm_esd_check buffer[4] = %d , buffer[5] = %d\n",buffer[4],buffer[5]);
        }
...
LCM_DRIVER KD101N37_40NA_A5_REVA_MIPI_drv = {
	.name = "KD101N37_40NA_A5_REVA_MIPI",
