...
int lcm_vgp_supply_enable(void)
{
	...
	ret = regulator_set_voltage(lcm_vgp, 3200000, 3200000);
	if (ret != 0) {
...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		params->dsi.PS=LCM_PACKED_PS_24BIT_RGB888;//LCM_LOOSELY_PS_18BIT_RGB666;//LCM_PACKED_PS_18BIT_RGB666;
		//params->dsi.word_count=1920*3; //720*3;	

		
		params->dsi.vertical_sync_active				= 8; //4; //6; //(12-4-4); //1;
		params->dsi.vertical_backporch					= 20; //6; //4; //6; //10;
		params->dsi.vertical_frontporch					= 7;//6; //4//6; //10;
		params->dsi.vertical_active_line				= FRAME_HEIGHT; 

		params->dsi.horizontal_sync_active				= 20;//4; //40; //44; //(120-40-40); //1;
		params->dsi.horizontal_backporch				= 59;//43;//44; //40; //44; //57;
		params->dsi.horizontal_frontporch				= 40;//45;//108;//44; //40; //44; //32;
		...
		//params->dsi.fbk_div =45;
        	params->dsi.PLL_CLOCK = 438;//480;//210;//480;//240;//LCM_DSI_6589_PLL_CLOCK_NULL; //LCM_DSI_6589_PLL_CLOCK_396_5;
...
LCM_DRIVER hn116wx1_100_dsi_edp_fhdanl_lcm_drv = 
{
    .name			= "hn116wx1_100_dsi_edp_fhdanl",
