...
static struct regulator *lcm_vibr;//add by tubao
...
void lcm_set_gpio_control_power(int val)
{
	if (val == 0) {
...
static int lcm_get_ldo_supply(struct device *dev)
{
	...
	pr_debug("LCM: lcm_get_ldo_supply is going¥n");
...
/****************************
Function:lcm_vibr_supply_enable
Description:control VIBR enable
******************************/
int lcm_vibr_supply_enable(void)
{
	int ret;
	unsigned int volt;
	...
	ret = regulator_set_voltage(lcm_vibr, 3300000, 3300000);
		if (ret != 0) {
		...
		volt = regulator_get_voltage(lcm_vibr);
		if (volt == 3300000)
			pr_err("LCM: check regulator lcm_vibr voltage=3300000 pass!\n");
		else
			pr_err("LCM: check regulator lcm_vibr voltage=3300000 fail! (voltage: %d)\n", volt);
	
		ret = regulator_enable(lcm_vibr);
		if (ret != 0) {
...
/****************************
Function:lcm_vpa_supply_enable
Description:control VPA enable
******************************/
int lcm_vpa_supply_enable(void)
{
	...
	if (NULL == lcm_vpa)
		return 0;
	//add by tubao ===========>>>>>>>>>>>>>    for contrl vpa power start start
	ret = regulator_set_voltage(lcm_vpa, 3300000, 3300000);
		...
		if (volt == 3300000)
			pr_err("LCM: check regulator lcm_vpa voltage=3300000 pass!\n");
		else
			pr_err("LCM: check regulator lcm_vpa voltage=3300000 fail! (voltage: %d)\n", volt);
	...
	//add by tubao  <<<<<<<<<<<<<<===============  for contrl vpa power start end
	return 0;//ret;
}

/****************************
Function:lcm_vgp_supply_enable
Description:control VGP enable
******************************/
int lcm_vgp_supply_enable(void)
{
	...
	if (volt == 3300000)
		pr_err("LCM: check regulator voltage=3300000 pass!¥n");
	else
		pr_err("LCM: check regulator voltage=3300000 fail! (voltage: %d)¥n", volt);
...
#endif
//add by tubao  lcm_vibr_supply_disable
/****************************
Function:lcm_vibr_supply_disable
Description:control VIBR disable
******************************/
int lcm_vibr_supply_disable(void)
{
	int ret = 0;
	...
	isenable = regulator_is_enabled(lcm_vibr);

	pr_info("LCM: lcm query regulator enable status[0x%d]\n", isenable);

	if (isenable) {
		ret = regulator_disable(lcm_vibr);
		if (ret != 0) {
	...
	isenable = regulator_is_enabled(lcm_vibr);
	if (!isenable)
		pr_err("LCM: lcm lcm_vibr regulator disable pass\n");
	}
...
/****************************
Function:lcm_vpa_supply_disable
Description:control VPA disable
******************************/
int lcm_vpa_supply_disable(void)
{
	...
	isenable = regulator_is_enabled(lcm_vpa);
	if (!isenable)
		pr_err("LCM: lcm lcm_vpa regulator disable pass\n");
	}
	//add by tubao   shutdown VPA power  end
	return ret;
}
/****************************
Function:lcm_vgp_supply_disable
Description:control VGP disable
******************************/
...
//	upmu_set_rg_vgp1_en(0x0);
	lcm_vibr_supply_disable();
//	lcm_vpa_supply_disable();
	lcm_vgp_supply_disable();
	MDELAY(10);
	lcm_set_gpio_control_power(0);
	MDELAY(20);
...
//	upmu_set_rg_vgp1_en(0x1);
	lcm_vibr_supply_enable();
	lcm_vpa_supply_enable();
	lcm_vgp_supply_enable();
	MDELAY(10);
	lcm_set_gpio_control_power(1);
	MDELAY(20);
#if 1
	lcm_set_gpio(1);
	MDELAY(20);
...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		params->dsi.vertical_backporch					= 8;//3; //6; //4; //6; //10;
		params->dsi.vertical_frontporch					= 8;//6; //4//6; //10;
		params->dsi.vertical_active_line				= FRAME_HEIGHT; 

		params->dsi.horizontal_sync_active				= 4;//5; //40; //44; //(120-40-40); //1;
		params->dsi.horizontal_backporch				= 132;//59;//44; //40; //44; //57;
		params->dsi.horizontal_frontporch				= 24;//16;//44; //40; //44; //32;
...
LCM_DRIVER KD101N37_40NA_A5_MIPI_t8611emmc_lcm_drv =
{
    .name			= "KD101N37_40NA_A5_MIPI_t8611emmc_lcm_drv",
