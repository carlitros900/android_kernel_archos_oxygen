...
int ovl_layer_switch_ex(DISP_MODULE_ENUM module, unsigned layer, unsigned int en, void *handle, int sec)
{
	int idx = ovl_index(module);
	int idx_offset = idx * DISP_OVL_INDEX_OFFSET;
	DDPDBG("ovl_layer_switch_ex mode= %d, layer= %d, sec= %d\n", module, layer, sec);
	ASSERT(layer <= 3);

	switch (layer) {
	case 0:
		#ifdef CONFIG_MTK_SEC_VIDEO_PATH_FULL_SUPPORT
		if ((handle != 0) && (idx == 1) && en && sec) {
			cmdqRecWriteSecureMask(handle, disp_addr_convert(idx_offset + DISP_REG_OVL_SRC_CON),
			CMDQ_SAM_DDP_REG_HDCP, en<<REG_FLD_SHIFT(SRC_CON_FLD_L0_EN), REG_FLD_MASK(SRC_CON_FLD_L0_EN));
			cmdqRecWriteSecureMask(handle, disp_addr_convert(idx_offset + DISP_REG_OVL_RDMA0_CTRL),
	...
	case 1:
		...
		if ((handle != 0) && (idx == 1) && en && sec) {
			cmdqRecWriteSecureMask(handle, disp_addr_convert(idx_offset + DISP_REG_OVL_SRC_CON),
			CMDQ_SAM_DDP_REG_HDCP, en<<REG_FLD_SHIFT(SRC_CON_FLD_L1_EN), REG_FLD_MASK(SRC_CON_FLD_L1_EN));
			cmdqRecWriteSecureMask(handle, disp_addr_convert(idx_offset + DISP_REG_OVL_RDMA1_CTRL),
			CMDQ_SAM_DDP_REG_HDCP, en, 0xffffffff);
		} else
		#endif
		{
	...
	case 2:
		...
		if ((handle != 0) && (idx == 1) && en && sec) {
			cmdqRecWriteSecureMask(handle, disp_addr_convert(idx_offset + DISP_REG_OVL_SRC_CON),
			CMDQ_SAM_DDP_REG_HDCP, en<<REG_FLD_SHIFT(SRC_CON_FLD_L2_EN), REG_FLD_MASK(SRC_CON_FLD_L2_EN));
			cmdqRecWriteSecureMask(handle, disp_addr_convert(idx_offset + DISP_REG_OVL_RDMA2_CTRL),
			CMDQ_SAM_DDP_REG_HDCP, en, 0xffffffff);
		} else
		#endif
		{
	...
	case 3:
		...
		if ((handle != 0) && (idx == 1) && en && sec) {
			cmdqRecWriteSecureMask(handle, disp_addr_convert(idx_offset + DISP_REG_OVL_SRC_CON),
			CMDQ_SAM_DDP_REG_HDCP, en<<REG_FLD_SHIFT(SRC_CON_FLD_L3_EN), REG_FLD_MASK(SRC_CON_FLD_L3_EN));
			cmdqRecWriteSecureMask(handle, disp_addr_convert(idx_offset + DISP_REG_OVL_RDMA3_CTRL),
			CMDQ_SAM_DDP_REG_HDCP, en, 0xffffffff);
		} else
		#endif
		{
...
static bool ovl1_in_hdmi_path(DISP_MODULE_ENUM module)
{
	bool in = false;

#ifdef CONFIG_MTK_HDMI_SUPPORT
	if (module == DISP_MODULE_OVL1 && is_hdmi_active())
		in = true;
#endif
	return in;
}
...
static int ovl_config_l(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *handle)
{
	...
	for (i = layer_min; i < layer_max; i++) {
		if (pConfig->ovl_config[i].layer_en != 0) {
			if (ovl_check_input_param(&pConfig->ovl_config[i]))
				continue;
			/* if AEE=1, assert layer addr must equal to asert_pa or reg value is not equal to
		...
		if (has_sec_layer && module == DISP_MODULE_OVL1 && ovl1_in_hdmi_path(module)) {
			ovl_layer_switch_ex(module, i % 4, pConfig->ovl_config[i].layer_en, handle,
