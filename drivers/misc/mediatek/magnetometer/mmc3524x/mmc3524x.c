...
#define MEMSIC_SINGLE_POWER 1
#define MMC_Pseudogyro  1
...
static int ECS_SaveData(int buf[28])//12
{
#if DEBUG
	struct i2c_client *client = this_client;
	struct mmc3524x_i2c_data *data = i2c_get_clientdata(client);
	...
	void *buff_out, int size_out, int *actualout)
{
...
#if DEBUG
	if (atomic_read(&data->trace) & MMC3524X_DEBUG)
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
			...
			gyrosensor_data->values[0] = sensor_data[12] * 32;
			gyrosensor_data->values[1] = sensor_data[13] * 32;
			gyrosensor_data->values[2] = sensor_data[14] * 32;
			gyrosensor_data->status = sensor_data[15];
			gyrosensor_data->value_divide = 32768;
	...
	void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
			...
			RV_data->values[0] = sensor_data[16] * 1;
			RV_data->values[1] = sensor_data[17] * 1;
			RV_data->values[2] = sensor_data[18] * 1;
			RV_data->status = sensor_data[19]; // fix w-> 0 w */
			RV_data->value_divide = 20000;
	...
	void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
			...
			gravity_data->values[0] = sensor_data[20] * 1;
			gravity_data->values[1] = sensor_data[21] * 1;
			gravity_data->values[2] = sensor_data[22] * 1;
			gravity_data->status = sensor_data[23];
			gravity_data->value_divide = 20000;
	...
	void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
			...
			LA_data->values[0] = sensor_data[24] * 1;
			LA_data->values[1] = sensor_data[25] * 1;
			LA_data->values[2] = sensor_data[26] * 1;
			LA_data->status = sensor_data[27];
			LA_data->value_divide = 20000;
...
static int mmc3524x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct hwmsen_object sobj_m, sobj_o,sobj_gy,sobj_vec,sobj_gravity,sobj_lineracc;
	...
	sobj_gy.self = data;
	sobj_gy.polling = 1;
	sobj_gy.sensor_operate = mmc3524x_gyroscope_operate;
	...
	sobj_vec.self = data;
	sobj_vec.polling = 1;
	sobj_vec.sensor_operate = mmc3524x_rotation_vector_operate;
	...
	sobj_gravity.sensor_operate = mmc3524x_gravity_operate;
err =hwmsen_attach(ID_GRAVITY, &sobj_gravity);
if(err)
	{
	printk(KERN_ERR "	sobj_gravity  attach fail = %d\n", err);
	goto exit_kfree;
	}
	
	sobj_lineracc.self = data;
	sobj_lineracc.polling = 1;
	sobj_lineracc.sensor_operate = mmc3524x_linear_accelration_operate;
	err =hwmsen_attach(ID_LINEAR_ACCELERATION, &sobj_lineracc);
	if(err)
	{
	printk(KERN_ERR "	sobj_lineracc  attach fail = %d\n", err);
	goto exit_kfree;
	}
