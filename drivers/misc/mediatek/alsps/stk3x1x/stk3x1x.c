 ...
 * configuration
*******************************************************************************/
#define PSCTRL_VAL	0x33		//0x31	/* ps_persistance=4, ps_gain=64X, PS_IT=0.391ms */
...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	 80 //240
	#define STK_LT_N_CT			20	//120  //
	#define STK_HT_N_CT		50	//	200 //100
...
printk("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz>>>>>%s\n",__FUNCTION__);
   alspsPltFmDev = get_alsps_platformdev();
/* gpio setting */
	pinctrl = devm_pinctrl_get(&alspsPltFmDev->dev);
	printk("llllllllllllllllllllllllllllllllll>>>>>%s\n",__FUNCTION__);
	...
	printk("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx>>>>>%s\n",__FUNCTION__);
/* eint request */
...
static int stk3x1x_init_client(struct i2c_client *client)
{
	...
	printk("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz\n");
	if(obj->first_boot == true)
	{	
		if(obj->hw->polling_mode_ps == 0 || obj->hw->polling_mode_als == 0)
		{
	...
	printk("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n");
	...
	printk("llllllllllllllllllllllllllllllllllllllllllllllllll\n");
/*	
...
*/	
	...
	printk("mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm\n");
	return 0;
}
...
static ssize_t stk3x1x_show_als(struct device_driver *ddri, char *buf)
{
	...
	{
		APS_DBG("stk3x1x_show_als   polling_mode_als !!\n");
	...
	APS_DBG("polling_mode_als   end !!%d \n",stk3x1x_obj->hw->polling_mode_als);
