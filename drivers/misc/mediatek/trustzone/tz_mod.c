...
DEFINE_SEMAPHORE(tz_cm_shrinker_sema);
DEFINE_SEMAPHORE(tz_cm_shrinker_finish_sema);

int tz_cm_shrinker_thread(void *data)
{
	sema_init(&tz_cm_shrinker_sema, 0);
	sema_init(&tz_cm_shrinker_finish_sema, 0);
	do {
		if (0 != down_interruptible(&tz_cm_shrinker_sema))
			continue;
						...
						struct shrink_control *sc)
{
	...
	up(&tz_cm_shrinker_sema);
	down(&tz_cm_shrinker_finish_sema);
