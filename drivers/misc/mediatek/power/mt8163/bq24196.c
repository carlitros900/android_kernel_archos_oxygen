...
{
	battery_log(BAT_LOG_CRTI, "[bq24196_shutdown] driver shutdown\n");
				  ...
				  unsigned char SHIFT)
{
	...
	battery_log(BAT_LOG_FULL, "[bq24196_read_interface] Reg[%x]=0x%x\n", RegNum, bq24196_reg);

	bq24196_reg &= (MASK << SHIFT);
	*val = (bq24196_reg >> SHIFT);

	battery_log(BAT_LOG_FULL, "[bq24196_read_interface] val=0x%x\n", *val);
				    ...
				    unsigned char SHIFT)
{
	...
	ret = bq24196_read_byte(RegNum, &bq24196_reg);
	battery_log(BAT_LOG_FULL, "[bq24196_config_interface] Reg[%x]=0x%x\n", RegNum, bq24196_reg);

	bq24196_reg &= ~(MASK << SHIFT);
	bq24196_reg |= (val << SHIFT);

	ret = bq24196_write_byte(RegNum, bq24196_reg);
	battery_log(BAT_LOG_FULL, "[bq24196_config_interface] write Reg[%x]=0x%x\n", RegNum, bq24196_reg);

	/* Check */
	/* bq24196_read_byte(RegNum, &bq24196_reg); */
	/* battery_log(BAT_LOG_FULL, "[bq24196_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24196_reg); */
...
void bq24196_dump_register(void)
{
	...
	battery_log(BAT_LOG_FULL, "[bq24196] ");
	for (i = 0; i < bq24196_REG_NUM; i++) {
		bq24196_read_byte(i, &bq24196_reg[i]);
		battery_log(BAT_LOG_FULL, "[0x%x]=0x%x ", i, bq24196_reg[i]);
	}
	battery_log(BAT_LOG_FULL, "\n");
}
...
static int bq24196_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq24196_driver_probe]\n");
...
static ssize_t store_bq24196_access(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	battery_log(BAT_LOG_CRTI, "[store_bq24196_access]\n");

	if (buf != NULL && size != 0) {
		battery_log(BAT_LOG_CRTI, "[store_bq24196_access] buf is %s and size is %zu\n", buf, size);
...
static int bq24196_user_space_probe(struct platform_device *dev)
{
	...
	battery_log(BAT_LOG_CRTI, "******** bq24196_user_space_probe!! ********\n");
...
static int __init bq24196_init(void)
{
		...
		battery_log(BAT_LOG_CRTI, "[bq24196_init] init start\n");
		...
		if (i2c_add_driver(&bq24196_driver) != 0)
			battery_log(BAT_LOG_CRTI, "[bq24196_init] failed to register bq24196 i2c driver.\n");
		else
			battery_log(BAT_LOG_CRTI, "[bq24196_init] Success to register bq24196 i2c driver.\n");

		/*bq24196 user space access interface*/
		ret = platform_device_register(&bq24196_user_space_device);
		if (ret) {
				battery_log(BAT_LOG_CRTI, "****[bq24196_init] Unable to device register(%d)\n", ret);
				return ret;
		}
		ret = platform_driver_register(&bq24196_user_space_driver);
		if (ret) {
				battery_log(BAT_LOG_CRTI, "****[bq24196_init] Unable to register driver (%d)\n", ret);
				return ret;
		}
		return 0;
}
